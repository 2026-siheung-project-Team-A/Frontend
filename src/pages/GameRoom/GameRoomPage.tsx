...
export function GameRoomPage() {
  ...
  const kickedByHost = useRoomStore((s) => s.kicked);
  ...
  // 호스트가 나를 강퇴함(room:kicked) — 강퇴 전용 안내 모달을 홈에서 띄우도록 kicked state 로 이동한다.
  useEffect(() => {
    if (kickedByHost) navigate('/', { state: { kicked: true } });
  ...
  // 방 삭제(closed)·게임 놓침(missed)·강퇴(kicked)는 위 useEffect 가 메인으로 보내므로 여기서 렌더할 화면이 없다(리다이렉트 대기).
  if (closed || missed || kickedByHost) return null;
