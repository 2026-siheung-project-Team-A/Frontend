<!--
  Pull Request Template — 룰렛 위젯 Frontend
  PR 제목은 Conventional Commits 형식을 따라 주세요.
  예) feat(roulette): 당첨 애니메이션 추가
  예) fix(widget): 모바일 뷰포트에서 위젯 잘림 수정
-->
...
- [ ] 로컬 수동 검증 완료 (`npm run dev`)
- [ ] 모바일 / 데스크톱 뷰 모두 확인
- [ ] 소켓 이벤트 변경 시 백엔드와 계약(이벤트명/페이로드) 확인 완료
...
| 디자인 변경 | [ ] Yes / [ ] No |
| 환경 변수 변경 | [ ] Yes / [ ] No |
| 백엔드 API 의존 변화 | [ ] Yes / [ ] No |
| 번들 크기 영향 | <!-- 예: +12KB / 변화 없음 --> |
...
- [ ] 테스트 전부 통과
- [ ] 콘솔 에러 / 경고 없음
- [ ] `.env` 변경 시 `.env.example` 및 README 갱신
