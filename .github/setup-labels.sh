...
LABELS=(
  ...
  # ── area/* (영역) ─────────────────────────────────────────────
  "area/config:C5DEF5:설정 파일 (package.json, vite.config 등)"
  "area/api:1D76DB:API 클라이언트 / 소켓 통신"
  "area/ui:F9D0C4:컴포넌트 / 페이지 / 스타일"
  "area/state:5319E7:상태 관리 / 커스텀 훅"
  "area/infra:CFD3D7:인프라 / 배포 (CI, Vercel)"
